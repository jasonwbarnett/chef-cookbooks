...
action :run do
  ...
  if mirror && distro
    ...
    base_repos.each do |repo|
      repos << "deb #{repo}"
      if node['bw_apt']['want_source']
        repos << "deb-src #{repo}"
      end
    end

    # update repos list and ensure base repos come first
    node.default['bw_apt']['repos'] = repos + node['bw_apt']['repos']
  end
