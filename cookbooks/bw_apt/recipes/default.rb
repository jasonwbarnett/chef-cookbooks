...
unless node.debian? || node.ubuntu?
  fail 'bw_apt is only supported on Debian and Ubuntu.'
end
...
Dir.glob('/etc/apt/sources.list.d/*').each do |f|
  file f do
    not_if { node['bw_apt']['preserve_sources_list_d'] }
    action :delete
  end
end

bw_apt_sources_list 'populate sources list' do
...
log 'periodic package cache update' do
  only_if do
    ...
    !::File.exists?(pkgcache) || (
      ::File.exists?(pkgcache) &&
      ::File.mtime(pkgcache) < Time.now - node['bw_apt']['update_delay'])
  end
  notifies :run, 'execute[apt-get update]'
end
