...
whyrun_safe_ruby_block 'validate_data' do
  block do
    node['bw_cron']['jobs'].to_hash.each do |name, data|
      if data['only_if']
        unless data['only_if'].class == Proc
          fail 'bw_cron\'s only_if requires a Proc'
        end
        unless data['only_if'].call
          Chef::Log.debug("bw_cron: Not including #{name} due to only_if")
          node.rm('bw_cron', 'jobs', name)
          next
        end
      end
      unless data['command']
        fail "bw_cron entry #{name} lacks a command"
      end
      unless data['time']
        fail "bw_cron entry #{name} lacks a time"
      end
      ...
      if data['exclusive']
        ...
        node.default['bw_cron']['jobs'][name]['command'] = command
      end
      ...
      if data['splaysecs']
        ...
        sleepnum = node.get_seeded_flexible_shard(Integer(data['splaysecs']),
                                                  data['command'])
        node.default['bw_cron']['jobs'][name]['splaycmd'] =
          "/bin/sleep #{sleepnum}; "
      else
        node.default['bw_cron']['jobs'][name]['splaycmd'] = ''
      end
...
template '/etc/cron.d/bw_crontab' do
  source 'bw_crontab.erb'
  owner 'root'
  group 'root'
  mode '0644'
end
