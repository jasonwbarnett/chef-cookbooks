  ...
  class Fstab
    ...
    def self.generate_base_fstab
      unless File.exists?(BASE_FILENAME) && File.size(BASE_FILENAME) > 0
        FileUtils.cp('/etc/fstab', '/root/fstab.before_bw_fstab')
        FileUtils.chmod(0400, '/root/fstab.before_bw_fstab')
    ...
    def self.get_in_maint_disks
      ...
      if age > 60 * 60 * 24 * 7
        Chef::Log.warn(
          "bw_fstab: Removing stale #{FB::Fstab::IN_MAINT_DISKS_FILENAME} " +
      ...
      unless disks.empty?
        Chef::Log.warn(
          "bw_fstab: Will skip in-maintenance disks: #{disks.join(' ')}",
        )
      end
      return disks
    end
    ...
    def self.autofs_parent(dir, node)
      get_autofs_points(node).each do |pt|
        Chef::Log.debug(
          "bw_fstab: Checking if #{dir} is within autofs tree at #{pt}",
        )
        if dir.start_with?(pt)
          Chef::Log.debug('bw_fstab: it is!')
          return pt
        end
      end
      false
    ...
    def self.label_to_device(label, node)
      ...
      Chef::Log.debug("bw_fstab: label #{label} is device #{d.keys[0]}")
      d.keys[0]
    end
    ...
    def self.uuid_to_device(uuid, node)
      ...
      Chef::Log.debug("bw_fstab: uuid #{uuid} is device #{d.keys[0]}")
      d.keys[0]
    end

    def self.canonicalize_device(device, node)
      Chef::Log.debug("bw_fstab: Canonicalizing #{device}")
