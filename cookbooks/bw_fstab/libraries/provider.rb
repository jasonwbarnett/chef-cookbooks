  ...
  module FstabProvider
    def mount(mount_data, in_maint_disks)
      if in_maint_disks.include?(mount_data['device'])
        Chef::Log.warn(
          "bw_fstab: Skipping mount of #{mount_data['mount_point']} because " +
      ...
      if mount_data['allow_mount_failure']
        Chef::Log.warn(
          "bw_fstab: Mounting #{mount_data['mount_point']} failed, but " +
    ...
    def umount(mount_point)
      Chef::Log.info("bw_fstab: Unmounting #{mount_point}")
    ...
    def remount(mount_point, with_umount)
      Chef::Log.info("bw_fstab: Remounting #{mount_point}")
      if with_umount
    ...
    def get_base_mounts
      ...
      ::File.read(FB::Fstab::BASE_FILENAME).each_line do |line|
        ...
        begin
          ...
          # later
          if node['bw_fstab']['mounts'].to_hash.any? do |_key, val|
               val['device'] == bits[0]
             end
            real_dev = bits[0]
          else
      ...
      Chef::Log.debug("bw_fstab: base mounts: #{mounts}")
      mounts
    end
    ...
    def should_keep(mounted_data, desired_mounts, base_mounts)
      ...
      desired_mounts.each do |_, desired_data|
        ...
        Chef::Log.debug("bw_fstab: --> Lets see if it matches #{desired_data}")
        ...
        if mounted_data['device'] && mounted_data['device'].start_with?('/dev/')
          if desired_device == mounted_data['device']
            Chef::Log.debug(
              "bw_fstab: Device #{mounted_data['device']} is supposed to be " +
      ...
      Chef::Log.debug('bw_fstab: --> OK, well is it a base mount?')
      if base_mounts[mounted_data['device']] &&
         base_mounts[mounted_data['device']]['mount_point'] ==
         mounted_data['mount']
        Chef::Log.debug(
          "bw_fstab: #{mounted_data['device']} on #{mounted_data['mount']} is" +
    ...
    def check_unwanted_filesystems
      ...
      devs_to_skip = node['bw_fstab']['umount_ignores']['devices'].dup
      dev_prefixes_to_skip =
        node['bw_fstab']['umount_ignores']['device_prefixes'].dup
      mounts_to_skip =
        node['bw_fstab']['umount_ignores']['mount_points'].dup
      fstypes_to_skip = node['bw_fstab']['umount_ignores']['types'].dup

      base_mounts = get_base_mounts
      # we're going to iterate over specified mounts a lot, lets dump it
      desired_mounts = node['bw_fstab']['mounts'].to_hash
      ...
      node['filesystem2']['by_pair'].to_hash.each do |_pair, mounted_data|
        ...
        unless mounted_data['mount']
          Chef::Log.debug(
            "bw_fstab: Skipping umount check for #{mounted_data['device']} " +
            "- it isn't mounted.",
          )
          next
        end
        ...
        if mounted_data.key?('inodes_used') && !mounted_data.key?('kb_used')
          Chef::Log.debug(
            'bw_fstab: Skipping mal-formed Chef 12 "df -i" entry ' +
            mounted_data.to_s,
          )
          next
        end
        ...
        if node['bw_fstab']['allow_umount']
          converge_by "unmount #{mounted_data['mount_point']}" do
            unmount(data['mount_point'])
          end
        else
          Chef::Log.warn(
            "bw_fstab: Would umount #{mounted_data['device']} from " +
            "#{mounted_data['mount']}, but " +
            'node["bw"]["bw_fstab"]["allow_umount"] is false',
          )
          Chef::Log.debug("bw_fstab: #{mounted_data}")
        end
      end
    end
    ...
    def tmpfs_mount_status(desired)
      ...
      if node['filesystem2']['by_pair'][key]
        ...
        if mounted['fs_type'] == 'tmpfs'
          Chef::Log.debug(
            "bw_fstab: tmpfs #{desired['device']} on " +
            "#{desired['mount_point']} is currently mounted...",
          )
          if compare_opts(desired['opts'], mounted['mount_options'])
            Chef::Log.debug('bw_fstab: ... with identical options.')
            return :same
          else
            Chef::Log.debug('bw_fstab: ... with different options.')
            return :remount
          end
        end
      end
      ...
      if node['filesystem2']['by_mountpoint'][desired['mount_point']]
        ...
        if mounted['fs_type'] == 'tmpfs'
          Chef::Log.warn(
            "bw_fstab: Treating #{mounted['devices']} on " +
          ...
          Chef::Log.debug("bw_fstab: #{desired} vs #{mounted}")
          if compare_opts(desired['opts'], mounted['mount_options'])
            Chef::Log.debug('bw_fstab: ... with identical options.')
            return :same
          else
            Chef::Log.debug(
              "bw_fstab: ... with different options #{desired['opts']} vs " +
        ...
        Chef::Log.warn(
          "bw_fstab: tmpfs is desired on #{desired['mount_point']}, but " +
    ...
    def mount_status(desired)
      ...
      # (and lose all their data) just to convert to bw_fstab. So we'll make
      ...
      if mounted
        Chef::Log.debug(
          "bw_fstab: There is an entry in node['filesystem2'] for #{key}",
        )
        ...
        if desired['type'] == mounted['fs_type'] ||
          ...
          Chef::Log.debug(
            "bw_fstab: FS #{desired['device']} on #{desired['mount_point']}" +
          ...
          Chef::Log.warn(
            "bw_fstab: Device #{desired['device']} is mounted at " +
      ...
      unless ['nfs', 'glusterfs'].include?(desired['type'])
        ...
        if device && device['mounts'] && !device['mounts'].empty?
          Chef::Log.warn(
            "bw_fstab: #{desired['device']} is at #{device['mounts']}, but" +
      ...
      if node['filesystem2']['by_mountpoint'][desired['mount_point']]
        ...
        Chef::Log.warn(
          "bw_fstab: Device #{desired['device']} desired at " +
    ...
    def check_wanted_filesystems
      ...
      # as appropriate.
      node['bw_fstab']['mounts'].to_hash.each do |_, desired_data|
        ...
        if desired_data['device'] == 'none'
          Chef::Log.warn('bw_fstab: We do not permit "none" devices, please ' +
                          'use a descriptive device name')
          next
        end

        if desired_data['type'] == 'swap'
          Chef::Log.debug('bw_fstab: We do not change swap from bw_fstab, ' +
                          'moving on...')
          next
        end
        ...
        when :same
          Chef::Log.debug(
            "bw_fstab: Skipping #{desired_data['mount_point']}; looks good!",
          )
          next
        when :missing
        ...
        when :conflict
          ...
          Chef::Log.info(
            "bw_fstab: Skipping #{desired_data['mount_point']} due to conflict",
          )
          next
        when :moved
          Chef::Log.info(
            "bw_fstab: Skipping #{desired_data['mount_point']} since it " +
            'moved. Moving filesystems is scary.',
          )
          next
        when :remount
          base_msg = "bw_fstab: Mountpoint #{desired_data['mount_point']} " +
                     'options changed'
          if node['bw_fstab']['enable_remount'] &&
             desired_data['enable_remount']
            Chef::Log.debug("bw_fstab: #{base_msg} - remounting")
