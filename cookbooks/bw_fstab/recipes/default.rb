...
whyrun_safe_ruby_block 'validate data' do
  block do
    uniq_devs = {}
    node['bw_fstab']['mounts'].to_hash.each do |name, data|
      # Handle only_if
      if data['only_if']
        unless data['only_if'].class == Proc
          fail 'bw_fstab\'s only_if requires a Proc'
        end
        unless data['only_if'].call
          Chef::Log.debug("bw_fstab: Not including #{name} due to only_if")
          node.rm('bw_fstab', 'mounts', name)
          next
        end
      end
      ...
      unless data['opts']
        node.default['bw_fstab']['mounts'][name]['opts'] = 'rw'
      end
      unless data['type']
        node.default['bw_fstab']['mounts'][name]['type'] = 'auto'
      end
      ...
      if auto
        fail "bw_fstab: Refusing to mount '#{name}' because the mount point " +
...
whyrun_safe_ruby_block 'get base mounts' do
  block do
    node.default['bw_fstab']['_basefilecontents'] =
