...
Attributes
----------
* node['bw_logrotate']['globals'][$CONFIG]
* node['bw_logrotate']['configs'][$NAME]
* node['bw_logrotate']['add_locking_to_logrotate']
...
and would get picked up by this logrotate recipe.
  size - specified by node['bw_logrotate']['globals']['size']
...
the resulting config file it would generate:
    ...
    default['bw_logrotate']['configs']['mcproxy'] = {
      'files' => [
...
machine using the following sample strucutre:
    ...
    node.default['bw_logrotate']['configs']['mylogfile'] = {
...
(/etc/newsyslog.d/bw_bsd_newsyslog.conf) is generated with output:
...
The `node['bw_logrotate']['add_locking_to_logrotate']` feature will *overwrite*
