...
if node.systemd?
  ...
  service 'mask swap unit' do # ~FC038
    not_if { node['bw_swap']['enabled'] }
    service_name swap_unit
    action [:stop, :mask]
  end

  service 'unmask swap unit' do # ~FC038
    only_if { node['bw_swap']['enabled'] }
    service_name swap_unit
    action [:unmask, :start]
  end
end
...
whyrun_safe_ruby_block 'validate swap size' do
  only_if do
    node['bw_swap']['size'] && node['bw_swap']['size'].to_i < 1024
  end
  block do
...
whyrun_safe_ruby_block 'validate resize' do
  only_if do
    node['bw_swap']['enabled'] && node['bw_swap']['size'] &&
    node['memory']['swap']['total'] != '0kB' &&
    (node['bw_swap']['size'].to_i - 4) > node['memory']['swap']['total'].to_i
  end
  block do
    fail 'bw_swap does not support increasing the size of a swap device'
  end
end
