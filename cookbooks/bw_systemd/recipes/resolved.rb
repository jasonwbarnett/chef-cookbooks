...
ruby_block 'enable nss-resolve' do
  only_if { node['bw_systemd']['resolved']['enable'] }
  block do
    node.default['bw_nsswitch']['databases']['hosts'].delete('dns')
    idx = node['bw_nsswitch']['databases']['hosts'].index('mymachines')
    node.default['bw_nsswitch']['databases']['hosts'].insert(idx + 1, 'resolve')
  end
end
...
service 'disable systemd-resolved' do
  not_if { node['bw_systemd']['resolved']['enable'] }
