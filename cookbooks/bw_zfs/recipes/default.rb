...
import_services.each do |svc|
  service svc do
    only_if { node['bw_zfs']['import_on_boot'] }
    action :enable
  end

  service "disable #{svc}" do
    not_if { node['bw_zfs']['import_on_boot'] }
    action :disable
  end
end

service 'zfs-zed' do
  only_if { node['bw_zfs']['enable_zed'] }
  action [:enable, :start]
end

service 'disable zfs-zed' do
  not_if { node['bw_zfs']['enable_zed'] }
  action [:stop, :disable]
end

service 'zfs-mount' do
  only_if { node['bw_zfs']['mount_on_boot'] }
  action :enable
end

service 'disable zfs-mount' do
  not_if { node['bw_zfs']['mount_on_boot'] }
  action :disable
end

service 'zfs-share' do
  only_if { node['bw_zfs']['share_on_boot'] }
  action :enable
end

service 'disable zfs-share' do
  not_if { node['bw_zfs']['share_on_boot'] }
  action :disable
end
